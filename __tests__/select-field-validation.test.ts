...
describe("Select Field Tests", () => {
  ...
  it("should throw error for invalid schema: select field with empty options array", () => {
    ...
    // Invalid data: selecting an option (empty options array)
    const invalidData = { companySize: "1-50" };
    expect(() => zodSchema.parse(invalidData)).toThrow();
  });
  ...
  it("should throw error for invalid schema: select field with non-object options", () => {
    ...
    // Invalid data: selecting an invalid option (invalid value type)
    const invalidData = { companySize: 100 };
    expect(() => zodSchema.parse(invalidData)).toThrow();
  });
  ...
  it("should throw error for invalid schema: select field with non-string option value", () => {
    const invalidSchema: any = {
      formTitle: "Select Field Form",
      formDescription: "Test for select field with invalid value type in options",
      fields: [
        {
          id: "companySize",
          type: "select",
    ...
    // Invalid data: selecting an option with a non-string value
    const invalidData = { companySize: 201 };
    expect(() => zodSchema.parse(invalidData)).toThrow();
  });
});
