...
describe("Email Field Tests", () => {
      ...
      fields: [
        {
          ...
          label: "Email",
          required: true,
          validation: { pattern:"^[a-zA-Z0-9._%+-]+@outlook\\.com$", message: "Only Outlook email addresses are allowed" },
        },
      ],
    };
  ...
  it("should throw error for invalid schema with invalid validation object", () => {
    ...
    expect(() => isValidSchema(invalidSchema)).not.toThrow(SchemaValidationError);
  });
